...
def local_line(side_mask):
    ...
    b1, b0 = np.polyfit(xs, ys, 1, w=np.sqrt(kw))   # polyfit squares its weights
    grid = np.linspace(xs.min(), xs.max(), 50)
    return grid, b0 + b1 * grid

gl, vl = local_line((x >= c - h_mse) & (x < c))
gr, vr = local_line((x >= c) & (x <= c + h_mse))
...
ax.scatter(b.rdplot_mean_x, b.rdplot_mean_y,
           s=sizes, color='grey', zorder=3)

# fit lines: red, clipped to the MSE-optimal window
in_h  = pl.rdplot_x.between(c - h_mse, c + h_mse)
left  = pl[in_h & (pl.rdplot_x < c)]
right = pl[in_h & (pl.rdplot_x >= c)]
ax.plot(left.rdplot_x,  left.rdplot_y,  color='red', lw=2)
ax.plot(right.rdplot_x, right.rdplot_y, color='red', lw=2)

ax.axvline(c, ls='--', color='k', lw=0.8)

#ax.plot(gl, vl, 'r-', lw=2); ax.plot(gr, vr, 'r-', lw=2)
