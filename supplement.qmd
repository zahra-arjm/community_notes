...
def robust_rdd(x,y,c):
    ...
    fit = rdrobust(y=y, x=x, c=c, kernel=KERNEL)
...
fit = robust_rdd(x, y, c)

tau   = float(fit.coef.loc["Conventional"].iloc[0])
p_rob = float(fit.pv.loc["Robust"].iloc[0])
ci_lo, ci_hi = fit.ci.loc["Robust"]

h_mse = float(fit.bws.loc["h", "left"])          # MSE-optimal bandwidth
n_eff = int(fit.N_h[0] + fit.N_h[1])             # effective N within bandwidth
...
τ̂ = `{python} f"{tau*100:+.1f}"`pp in the probability of writing a second
...
out = rdplot(y=y, x=x, c=c, kernel='triangular', 
...
ax.axvline(c, ls='--', color='k', lw=0.8)
